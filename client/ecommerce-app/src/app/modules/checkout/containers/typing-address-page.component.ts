...
@Component({
  ...
  template: `
  <app-address-view [address]="addressForm" [editAddress]="editAddress$ | async"   [countries]="listCountries$ | async"  (save)="saveAddress()" (update)="updateAddress()" (cancel)="cancelEdit()"></app-address-view>`,
})
export class TypingAddressPageComponent implements OnInit, OnDestroy {   
  ...
  ngOnInit(): void {
   ...
   this.subscription = this.store.select(selectEditAddress).pipe(take(1)).subscribe((address: Address | null) => {
      if(address!=null){        
      //set values in case of  
  ...
  updateAddress(){
    this.subscription = this.store.select(selectEditAddress).pipe(take(1)).subscribe(addressFromStore => {   
      const address = new AddressBuilder(this.addressForm.value)
